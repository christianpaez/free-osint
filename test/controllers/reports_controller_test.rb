...
class ReportsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should return 404 for non-existent report' do
    get report_url(id: 999_999)
    assert_response :not_found
  end
  test 'should create report job' do
    post reports_url, params: { domain: 'new-domain.com' }
    assert_response :ok
    json = response.parsed_body
    assert_equal 'new-domain.com', json['domain']
    assert_equal 'processing', json['status']
  end

  test 'should return existing report' do
    post reports_url, params: { domain: @report.company_domain }
    assert_response :ok
    json = response.parsed_body
    assert_equal 'completed', json['status']
    assert_equal @report.id, json['report_id']
  end

  test 'should return error when domain missing' do
    post reports_url
